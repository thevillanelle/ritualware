...
const APPS = [
  { name: 'Ritualwear',    url: 'https://wear.ritualware.app',    color: '#D4919A', desc: 'Style Oracle' },
  { name: 'Glow Up',       url: 'https://glowup.ritualware.app',  color: '#8FA688', desc: 'Beauty Audit' },
  { name: 'Ritualwhere?',  url: 'https://where.ritualware.app',   color: '#A89BC4', desc: 'City Guide' },
  { name: "m'atelier",     url: 'https://studio.ritualware.app',  color: '#C8A86B', desc: 'Studio' },
  { name: 'Ritualware',    url: 'https://ritualware.app',         color: '#C4717A', desc: 'Marketing site' },
]
...
export default function Status() {
  const [results, setResults] = useState(
    APPS.map(app => ({ ...app, status: 'checking', ms: null, checkedAt: null }))
  )
  ...
  const checkAll = useCallback(async () => {
    ...
    const updated = await Promise.all(
      APPS.map(async (app) => {
        ...
        try {
          // Use a HEAD request via no-cors — we can't read the body but we
          // can detect whether the server responded at all
          await fetch(app.url, { method: 'HEAD', mode: 'no-cors', cache: 'no-store' })
          const ms = Math.round(performance.now() - start)
          return { ...app, status: 'up', ms, checkedAt: new Date() }
        } catch {
          return { ...app, status: 'down', ms: null, checkedAt: new Date() }
        }
      })
    )
