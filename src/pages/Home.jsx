...
export default function Home() {
  return (
    <main>
      <section className="relative flex items-center overflow-hidden pt-28 pb-16 md:pt-40 md:pb-24">
        ...
        <div className="relative z-10 w-full max-w-4xl mx-auto px-6 md:px-16">
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.7 }}
            className="font-display leading-[1.05] text-ink mb-8"
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}
            ...
            {chips.map(chip => (
              <Link key={chip.to} to={chip.to}
                className="font-mono text-sm text-ink bg-cream-alt border border-ink/15 px-5 py-2.5 rounded-full hover:border-rose hover:text-rose transition-colors">
                {chip.label}
              </Link>
            ))}
          </motion.div>
